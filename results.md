...
精度が同じ（≈0.17）でも BC は異なる。これは BC が精度の代替でないことを示す。

- **V1+ノイズ** の BC が先に落ちる理由: ガウスノイズはカテゴリ内分散（分母）を均一に増加させる → BC が低下する
- **HVC（ノイズなし）** が同精度で高 BC の理由: HVC のシグナル全体は弱いが、存在するシグナルはカテゴリ方向に集中 → カテゴリ内分散が小さく保たれ BC が高い

**BC が精度より多く測定していること**

```
識別精度 = 「デコードされた表現がカテゴリを正しく識別できるか」（相対評価）
BC       = 「カテゴリ内のシグナルが prior に比べてどれだけカテゴリ方向に集中しているか」（絶対スケール）

→ V1+ノイズ(σ=0.75) vs HVC:
  精度は同じ → 「どちらもカテゴリを約17%正しく識別できる」
  BC が違う → 「HVC のシグナルはよりカテゴリ集中的;
               V1+ノイズのシグナルは一様なノイズで希釈されている」
```

**論文への示唆**: BC と識別精度は高相関だが、精度が同じでも BC は異なりうる。BC はシグナルの「カテゴリ集中度」を prior との比で測定しており、識別精度では捉えられない次元を持つ。
