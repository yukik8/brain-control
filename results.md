...
答え:
  BC が示す新情報 = 「絶対的な prior 支配度」

  識別精度 V1=0.603 → 「V1 は 60% の試行でカテゴリを正しく識別できる」
  BC       V1=1.107 → 「V1 でさえ prior より 10% しか上回っていない」
                        「再構成の 90% 以上は generator prior が決めている」

  識別精度はどの ROI が「より良い」かを示すが、
  BC は「どの ROI も prior 支配で、best case でも 10% の脳制御しかない」を示す。

  → BC がなければ「V1 で 60% 識別できた = 良い再構成」と誤解される。
  → BC があれば「60% 識別できても BC=1.1 = 再構成は prior 主導」と正確に評価できる。
```

**論文への示唆**: BC と識別精度の相関は「両者が同じ信号を捉えている」ことを示す（妥当性の確認）。しかし BC の付加価値は **絶対的スケール**にある。識別精度が高くても BC ≈ 1.0 なら、それは prior-dominated な再構成である。BC を使わないと、全ROI が「良い」ように見えてしまう（識別精度が chance 以上なので）が、BC は「全 ROI が prior 支配」という事実を暴く。
