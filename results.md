...
```
BC = Var(broken) / Var(preserved)

Var(preserved) = 同カテゴリの試行内での特徴量の分散（カテゴリ内 trial-to-trial variability）
Var(broken)    = 試行インデックスをシャッフルした後の「カテゴリスロット」内の分散の期待値

BC = 1  → ランダム摂動と正しい対応付けで分散が同じ → 脳活動の情報は特徴量に伝わっていない
BC > 1  → 正しい対応付けのときのほうが分散が小さい → 脳が再構成をある程度制御している
```

**情報理論的解釈**: ガウス近似下で $I(c; \hat{\mathbf{x}}) \approx \frac{1}{2} \log \text{BC}$
→ BC = 1 は相互情報量ゼロ（情報なし）、BC > 1 はカテゴリ情報の保持を示す。

詳細な理論定義: `theory_bc_definition.md`
...
### Exp16b: BC の感度優位性（ノイズカーブ）

同データから、BC は識別精度より **敏感にノイズ劣化を反映する** ことを可視化。

![BC 感度優位性](outputs/exp16b_noise_sensitivity.png)

**観察**:
- 識別精度（赤破線）: σ=0.25 付近から緩やかに低下
- BC（青実線）: σ=0.25 でより急峻に低下 → σ=0.5 の時点で BC は中間点を超えて低下
